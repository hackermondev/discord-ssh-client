...
const commandCallback = async (
  ...
  var guildData: GuildData | undefined = await client.db.getGuildData(
    message.guild.id
  );
  ...
  if (!name) {
    ...
    var embed = new MessageBuilder(message)
      ...
      .setDescription(
        `Change a value with \`${guildData.guild_data.prefix}guildsettings <name> <value>\` `
      );
  ...
  if (name == 'prefix') {
    ...
    if (!newPrefix) {
      var error = new MessageBuilder(message)
        ...
        .setDescription(
          `You need to specify what prefix it should be changed to.`
        )
        .setColor(config.colors.error);
    ...
    try {
      ...
      await client.db.query(
        `UPDATE guilds SET guild_data=$1 WHERE guild_id=$2`,
        [guildData.guild_data, message.guild.id]
      );
