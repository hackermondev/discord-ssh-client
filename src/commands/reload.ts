...
const commandCallback = async (client: Client, message: Message, args: string[]) => {
    ...
    if (cmd == undefined) {
        var error = new MessageBuilder(message)
            ...
            .setDescription(`You need to provide a valid command to be reloaded. Please run this command again.`)
    ...
    if (!botCommand) {
        var error = new MessageBuilder(message)
            .setTitle(`Reload Command Error`)
            .setDescription(`Command \`${cmd}\` was not found. Please run this command again with a valid command.`)
            .setColor(config.colors.error)
    ...
    var embed = new MessageBuilder(message)
        ...
        .setDescription(`Command \`${cmd}\` was successfully reloaded. You can run this command with the updated code.`)
