...
const commandCallback = async (client: Client, message: Message, args: string[]) => {
    ...
    if (commandToBeExecuted == ``) {
        var error = new MessageBuilder(message)
            .setTitle(`SSH Command Error`)
            .setDescription(`You need to provide a valid command to be executed. Please run the command again. Example: \`${config.botConfig.prefix}command 69.69.69.69:22 your_auth_key ls\` `)
            .setColor(config.colors.error)
    ...
    try {
        ...
        try {
            ...
            var success = new MessageBuilder(message)
                ...
                .setDescription(`We've connected to the ssh server. Running command \`${commandToBeExecuted}\` `)
                .setColor(config.colors.success)

            await connectingMessage.edit(success)
            c.exec(commandToBeExecuted, {
                env: {
                ...
                stream.on('close', (code: Number) => {
                    ...
                    if (code == 1) {
                        var error = new MessageBuilder(message)
                            ...
                            .setDescription(`The ssh command was executed on the server. The status code was ${code}. Output:\n\n \`${output}\` `)
