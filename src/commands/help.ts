...
const commandCallback = async (
  ...
  if (!command) {
    ...
    commandsText += `\n\nRun \`${
      guildData ? guildData.guild_data.prefix : config.botConfig.prefix
    }help <command>\` to learn more information`;
    ...
    if (!commandInfo) {
      ...
      embed.setDescription(
        `\`${
          guildData ? guildData.guild_data.prefix : config.botConfig.prefix
        }${commandInfo.usage || commandInfo.name}\` \n${
          commandInfo.description
        }\n\nAliases: ${commandInfo.aliases.map((a: string) => {
          return ` \`${a}\` `;
        })}`
      );
    }
