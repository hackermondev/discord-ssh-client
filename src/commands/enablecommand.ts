...
const commandCallback = async (client: Client, message: Message, args: string[]) => {
    ...
    if(!guildSettings?.guild_data.disabledCommands.includes(cmd.name)){
        var error = new MessageBuilder(message)
            ...
            .setDescription(`This command is not disabled. You cannot enable a command that is already enabled.`)
            .setColor(config.colors.error)
    ...
    await Promise.all(guildSettings.guild_data.disabledCommands.map((disabled, index)=>{
