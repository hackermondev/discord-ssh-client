    ...
    try {
        ...
        try {
            ...
            var success = new MessageBuilder(message)
                ...
                .setDescription(`We've connected to the ssh server. Starting terminal...`)
            ...
            c.shell((err: Error | undefined, stream: Channel) => {
                if (err) {
                    ...
                    var error = new MessageBuilder(message)
                        .setTitle(`SSH Command Error`)
                        .setDescription(`An error occured while starting the shell.\n\n \`${err}\``)
                        .setColor(config.colors.error)
                ...
                stream.on('close', async ()=>{
                    ...
                    var embed = new MessageBuilder(message)
                        ...
                        .setColor(config.colors.success)
                        .setDescription(`SSH shell has been closed. The text below is the log of the terminal\n\n \`\`\`${utils.parseTerminalOutput(output)}\`\`\` \n\n[Pastebin](${pasteURL})`)
                ...
                var interval: any = setInterval(() => {
                    ...
                    var embed = new MessageBuilder(message)
                        ...
                        .setDescription(`SSH shell is current ongoing. Type something in the chat to see it entered in the terminal or enter \`cancel\` to exit the terminal! The output below will get updated every few seconds with updated from the shell.\n\n \`\`\`${utils.parseTerminalOutput(output)}\`\`\` `)

                    lastContent = output
                    connectingMessage.edit(embed)
                }, 100)

                const collector: MessageCollector = message.channel.createMessageCollector((m)=>{
