  ...
  try {
    ...
    if (!id) {
      var error = new MessageBuilder(message)
        .setTitle(`Error`)
        .setDescription(
          `You need to provide a valid auth key to delete from our database.`
        )
    ...
    var query = await client.db.query(`SELECT * FROM auth WHERE hash = $1`, [
      id
    ]);
    ...
    if (item.created_by != message.author.id) {
      var error = new MessageBuilder(message)
        .setTitle(`Error`)
        .setDescription(
          `You cannot delete this key! This is not your auth key!`
        )
        .setColor(config.colors.error);
    ...
    var embed = new MessageBuilder(message)
      ...
      .setDescription(
        `That auth key was successfully deleted from our database.`
      )
    ...
    var error = new MessageBuilder(message)
      .setTitle(`Error`)
      .setDescription(
        `An error occured while fetching the keys from our database. Please contact the owners for more information.`
      )
      .setColor(config.colors.error);
