...
function parseTerminalOutput(output: string): string{
    output = output.split('[01;34m').join('').split('[0m').join('').split('[01;32m').join('').split('[33m').join('').split('[39m').join('')
...
function connectToSSHAndReturnClient(hostData: ServerData, auth: AuthData, message: Message): Promise<SSHClient> {
        ...
        try{
            ...
            c.once(`change password`, async (prompt: string, done: Function)=>{
                ...
                var embed = new MessageBuilder(message)
                    ...
                    .setDescription(`The server requested that will asked you to change your password. Please change the new password in this DM, otherwise say \`cancel\`.\n\nPrompt: \`${prompt}\` `)
                ...
                var passwordCollector: Collection<string, Message> = await m.channel.awaitMessages(()=>{ return true }, { 
                    max: 1,
                    time: 300000
                })
                ...
                if(!password || password.content.toLowerCase() == `cancel`){
                    c.end()
                    reject(new Error(`Password change was canceled or user didn't respond with password.`))

                    return message.author.send(`Password change was canceled or user didn't respond with password. Closing server.`)
                }
...
export default { parseServerData, connectToSSHAndReturnClient, parseTerminalOutput } 